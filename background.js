...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  ...
  if (info.menuItemId === MENU_HIGHLIGHT) {
    chrome.tabs.sendMessage(tab.id, { type: "CF_HIGHLIGHT_SELECTION" });
  }

  if (info.menuItemId === MENU_REMOVE) {
    chrome.tabs.sendMessage(tab.id, { type: "CF_REMOVE_HIGHLIGHT" });
  }
});
...
chrome.commands.onCommand.addListener(async (command) => {
  ...
  if (!tab?.id || !tab.url?.startsWith("https://codeforces.com/")) {
    return;
  }
